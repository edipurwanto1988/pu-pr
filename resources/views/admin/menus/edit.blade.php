...
<div class="">
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Menu</h1>
    </div>
        ...
        <form action="{{ route('admin.menus.update', $menu) }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="title" value="{{ $menu->title }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">URL</label>
                    <div class="flex gap-2">
                        <input type="text" name="url" id="url-input" value="{{ $menu->url }}" class="flex-1 rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required placeholder="/url-atau-https://...">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="flex-1 relative">
                            <input type="text" name="icon" id="icon-input" value="{{ $menu->icon }}" placeholder="ri-home-line" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 pr-10">
                            @if($menu->icon)
                                <span class="absolute right-3 top-1/2 -translate-y-1/2"><i class="{{ $menu->icon }} text-lg text-gray-600"></i></span>
                            @endif
                        </div>
                ...
                <div>
                    ...
                    <select name="parent_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        ...
                        @foreach($parentMenus as $pm)
                            <option value="{{ $pm->id }}" {{ $menu->parent_id == $pm->id ? 'selected' : '' }}>{{ $pm->title }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="target" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="_self" {{ $menu->target === '_self' ? 'selected' : '' }}>Same Tab (_self)</option>
                        <option value="_blank" {{ $menu->target === '_blank' ? 'selected' : '' }}>New Tab (_blank)</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Urutan</label>
                    <input type="number" name="order" value="{{ $menu->order }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div>
