        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" value="{{ $product->name }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                </div>
                <div>
                    ...
                    <select name="umkm_profile_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                        @foreach($umkm as $u)
                            <option value="{{ $u->id }}" {{ $product->umkm_profile_id == $u->id ? 'selected' : '' }}>{{ $u->business_name }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="price" value="{{ $product->price }}" step="0.01" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1"><i class="ri-file-text-line mr-1"></i>Deskripsi</label>
                    <textarea name="description" id="description" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{!! $product->description !!}</textarea>
                </div>
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1">Tambah gambar baru. Gambar pertama akan menjadi gambar utama.</p>
                </div>
            </div>
...
function deleteImage(btn, imageId) {
    ...
    fetch('{{ route("admin.products.images.destroy", [$product->id, "__IMGID__"]) }}'.replace('__IMGID__', imageId), {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
            'Accept': 'application/json'
        ...
        if (response.ok) {
            ...
            if (container) {
                const first = container.querySelector('.relative');
                if (first) {
                    const badge = first.querySelector('span');
                    if (!badge) {
                        const s = document.createElement('span');
                        s.className = 'absolute bottom-1 left-1 bg-[#ca4e33] text-white text-[10px] px-1.5 py-0.5 rounded';
                        s.textContent = 'Utama';
                        first.appendChild(s);
                    }
                }
...
document.getElementById('image-input').addEventListener('change', function(e) {
    ...
    const previewId = 'new-image-preview';
    let container = document.getElementById(previewId);
    if (!container) {
        container = document.createElement('div');
        container.id = previewId;
        container.className = 'flex flex-wrap gap-3 mb-2';
    ...
    Array.from(e.target.files).forEach(function(file, index) {
        ...
        reader.onload = function(ev) {
            ...
            wrapper.innerHTML = '<img src="' + ev.target.result + '" class="w-24 h-24 object-cover rounded-lg border">';
