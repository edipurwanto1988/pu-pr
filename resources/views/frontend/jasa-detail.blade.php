    ...
    <div class="border-b border-gray-100 bg-gray-50/50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
            <nav class="flex items-center gap-2 text-xs md:text-sm text-gray-500 overflow-x-auto whitespace-nowrap pb-1 md:pb-0">
                ...
                <a href="{{ route('jasa') }}" class="hover:text-[#ca4e33] transition-colors">Jasa</a>
                <i class="ri-arrow-right-s-line text-gray-300"></i>
                            ...
                            <div class="aspect-square md:aspect-[4/3] bg-gray-100 rounded-2xl overflow-hidden shadow-sm border border-gray-100" id="mainImageWrap">
                                ...
                                @else
                                    <div class="w-full h-full flex flex-col items-center justify-center text-gray-300 bg-gray-50">
                                        ...
                                        <p class="text-sm">Tidak ada foto layanan</p>
                                    </div>
                                @endif
                            </div>
                ...
                <div class="lg:col-span-5 flex flex-col">
                    <div class="mb-6">
                        <div class="flex items-center gap-2 mb-4">
                            @if($product->category)
                                <a href="{{ route('jasa', ['category' => $product->category->id]) }}" class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-600 text-xs font-bold uppercase tracking-wider rounded-full hover:bg-blue-100 transition-colors">
                                    <i class="ri-service-line"></i> {{ $product->category->name }}
                                </a>
                            @endif
                            <span class="text-xs text-gray-400 bg-gray-100 px-2.5 py-1 rounded-full flex items-center gap-1">
                                <i class="ri-checkbox-circle-line"></i> Layanan Terverifikasi
                            </span>
                        </div>
                        ...
                        @if($product->price)
                            <div class="flex items-baseline gap-2 mb-6">
                                <span class="text-gray-500 text-lg font-medium">Mulai dari</span>
                                <span class="text-3xl md:text-4xl font-black text-[#ca4e33]">Rp {{ number_format($product->price, 0, ',', '.') }}</span>
                            </div>
                        @else
                            <div class="mb-6">
                                <span class="text-lg font-bold text-gray-500 italic">Harga Hubungi Penyedia</span>
                            </div>
                        @endif
                ...
                <div class="lg:col-span-6 space-y-12">
                    ...
                    <div id="deskripsi" class="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100">
                        <div class="flex items-center gap-3 mb-8">
                            ...
                            <h2 class="text-2xl md:text-3xl font-bold text-gray-900">Deskripsi Layanan</h2>
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                        <h3 class="font-bold text-gray-900 mb-4 flex items-center gap-2">
                            <i class="ri-information-line text-[#ca4e33]"></i> Detail Jasa
                        </h3>
                        <ul class="space-y-3 text-sm">
                            <li class="flex justify-between py-2 border-b border-gray-50">
                                <span class="text-gray-500">Kategori Jasa</span>
                                <span class="font-medium text-gray-900">{{ $product->category?->name ?? '-' }}</span>
                            </li>
                            <li class="flex justify-between py-2 border-b border-gray-50">
                                <span class="text-gray-500">Tipe Penyedia</span>
                    ...
                    <div class="sticky top-24 space-y-6">
                        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div class="p-6 border-b border-gray-50 bg-gray-50/30">
                                <h3 class="text-lg font-bold text-gray-900 mb-4">Informasi Penyedia</h3>
                                @if($product->umkmProfile)
                                    <div class="flex items-center gap-4">
                                        ...
                                        @else
                                            <div class="w-16 h-16 bg-white rounded-xl flex items-center justify-center text-blue-600 border-2 border-gray-100 shadow-sm">
                                                <i class="ri-user-settings-line text-2xl"></i>
                                            </div>
                                        @endif
                                        <div>
                                            <h4 class="font-bold text-gray-900 leading-tight">
                                                <a href="{{ route('umkm.detail', $product->umkmProfile->slug) }}" class="hover:text-blue-600 transition-colors">{{ $product->umkmProfile->business_name }}</a>
                                            </h4>
                                            <span class="text-[10px] font-bold bg-blue-600/10 text-blue-600 px-2 py-0.5 rounded uppercase mt-1 inline-block">{{ $product->umkmProfile->type }}</span>
                                        </div>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="p-6 space-y-6">
                                @if($product->umkmProfile)
                                    ...
                                    <div class="pt-6 border-t border-gray-50 flex flex-col gap-2">
                                        ...
                                        @if($product->umkmProfile->whatsapp)
                                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $product->umkmProfile->whatsapp) }}" class="flex items-center justify-center gap-2 w-full py-2.5 text-gray-600 hover:text-blue-600 text-sm font-semibold transition-colors">
                        ...
                        <div class="bg-blue-50 border border-blue-100 p-6 rounded-3xl flex items-start gap-4">
                            <div class="w-12 h-12 shrink-0 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-200">
                                <i class="ri-customer-service-2-line text-2xl"></i>
                            </div>
                            <div>
                                <h4 class="font-bold text-gray-900 mb-1">Dukung Jasa Lokal</h4>
                                <p class="text-xs text-gray-500 leading-relaxed">Gunakan jasa dari pelaku usaha di sekitar Anda untuk memajukan ekonomi Pekanbaru.</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="py-16 md:py-24 border-t border-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-10">
                <div>
                    <h2 class="text-3xl font-black text-gray-900 mb-2">Jasa Terkait</h2>
                    <p class="text-gray-500">Mungkin Anda juga membutuhkan layanan-layanan berikut.</p>
                </div>
                <a href="{{ route('jasa') }}" class="text-blue-600 font-bold flex items-center gap-1 hover:gap-2 transition-all">
                    Lihat Semua Jasa <i class="ri-arrow-right-line"></i>
                </a>
            </div>
                ...
                @foreach($relatedProducts as $item)
                    <div class="group bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2">
                        <a href="{{ route('jasa.detail', $item->slug) }}" class="block">
                            ...
                            <div class="p-6">
                                <p class="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-1">{{ $item->umkmProfile?->business_name }}</p>
                                <h3 class="font-bold text-gray-900 group-hover:text-blue-600 transition-colors mb-3 truncate">{{ $item->name }}</h3>
                                <div class="flex items-center justify-between">
                                    @if($item->price)
                                        <p class="font-black text-gray-900 text-sm"><span class="text-[10px] font-normal text-gray-400 block -mb-1">Mulai dari</span> Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                                    @else
                                        <p class="text-xs font-bold text-gray-400 uppercase italic">Hubungi Penyedia</p>
                                    @endif
                                    <div class="w-8 h-8 rounded-full bg-gray-50 group-hover:bg-blue-600 group-hover:text-white flex items-center justify-center text-gray-400 transition-all">
...
function shareProduct() {
    if (navigator.share) {
        navigator.share({
            title: '{{ $product->name }}',
            text: 'Lihat jasa {{ $product->name }} dari {{ $product->umkmProfile?->business_name }} di PUPR Pekanbaru',
